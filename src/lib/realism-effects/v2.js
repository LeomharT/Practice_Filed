import { Effect, NormalPass, Pass, RenderPass, Selection } from 'postprocessing';
import {
  ClampToEdgeWrapping,
  Clock,
  Color,
...
class TemporalReprojectMaterial extends ShaderMaterial {
  constructor(textureCount = 1) {
    let finalFragmentShader = fragmentShader$7
...
const isGroundProjectedEnv = (c) => {
  ...
  return (_c$material$fragmentS = c.material.fragmentShader) == null
    ? void 0
    : _c$material$fragmentS.includes(
...
class TemporalReprojectPass extends Pass {
  constructor(
    ...
    this.fullscreenMaterial.defines.inputType =
      (_indexOf = ['diffuseSpecular', 'diffuse', 'specular'].indexOf(options.inputType)) !== null && _indexOf !== void 0
        ? _indexOf
        : 1;
  ...
  setSize(width, height) {
    ...
    for (let i = 0; i < this.textureCount; i++) {
      ...
      const accumulatedTexture =
        (_this$overrideAccumul = this.overrideAccumulatedTextures[i]) !== null && _this$overrideAccumul !== void 0
          ? _this$overrideAccumul
...
class TRAAEffect extends Effect {
  ...
  setSize(width, height) {
    ...
    (_this$temporalReproje = this.temporalReprojectPass) == null
      ? void 0
      : _this$temporalReproje.setSize(width, height);
  }
...
class DenoiserComposePass extends Pass {
  constructor(camera, textures, gBufferTexture, depthTexture, options = {}) {
    ...
    this.fullscreenMaterial = new ShaderMaterial({
      ...
      defines: {
        inputType:
          (_indexOf = ['diffuseSpecular', 'diffuse', 'specular'].indexOf(options.inputType)) !== null &&
          _indexOf !== void 0
            ? _indexOf
            : 0,
      },
  ...
  render(renderer) {
    for (let i = 0; i < 2 * this.iterations; i++) {
      ...
      this.fullscreenMaterial.uniforms['inputTexture'].value =
        i === 0 ? this.textures[0] : inputRenderTarget.texture[0];
...
class Denoiser {
  constructor(scene, camera, texture, options = defaultDenosierOptions) {
    ...
    this.options = options;
    this.velocityDepthNormalPass =
      (_options$velocityDept = options.velocityDepthNormalPass) !== null && _options$velocityDept !== void 0
        ? _options$velocityDept
    ...
    if (this.options.denoiseMode === 'full' || this.options.denoiseMode === 'denoised') {
      ...
      this.denoisePass.setGBufferPass(
        (_options$gBufferPass = options.gBufferPass) !== null && _options$gBufferPass !== void 0
          ? _options$gBufferPass
          : this.velocityDepthNormalPass,
      );
      this.temporalReprojectPass.overrideAccumulatedTextures = this.denoisePass.renderTargetB.texture;
    }

    const composerInputTextures =
      (_this$denoisePass$tex = (_this$denoisePass = this.denoisePass) == null ? void 0 : _this$denoisePass.texture) !==
        null && _this$denoisePass$tex !== void 0
        ? _this$denoisePass$tex
        : textures;
  ...
  render(renderer, inputBuffer = null) {
    ...
    if (this.options.inputType !== 'diffuseSpecular') {
      ...
      (_this$denoiserCompose3 = this.denoiserComposePass) == null
        ? void 0
        : _this$denoiserCompose3.setSceneTexture(inputBuffer.texture);
    }
...
class SSGIEffect extends Effect {
      ...
      Object.defineProperty(this, key, {
        ...
        set(value) {
          ...
          switch (key) {
            ...
            case 'specularPhi':
              if (
                (_this$denoiser$denois = this.denoiser.denoisePass) != null &&
                _this$denoiser$denois.fullscreenMaterial.uniforms[key]
              ) {
  ...
  setSize(width, height, force = false) {
    ...
    (_this$cubeToEquirectE = this.cubeToEquirectEnvPass) == null
      ? void 0
      : _this$cubeToEquirectE.setSize(width, height);
    this.lastSize = {
      width,
      height,
  ...
  update(renderer, inputBuffer) {
    ...
    this.uniforms.get('inputTexture').value =
      (_this$outputTexture$ = this.outputTexture[0]) !== null && _this$outputTexture$ !== void 0
        ? _this$outputTexture$
        : this.outputTexture;
...
class AOEffect extends Effect {
  constructor(composer, camera, scene, aoPass, options = defaultAOOptions) {
    ...
    if (options.useNormalPass || options.normalTexture) {
      ...
      const normalTexture =
        (_options$normalTextur = options.normalTexture) !== null && _options$normalTextur !== void 0
          ? _options$normalTextur
  ...
  update(renderer) {
    ...
    const hasTRAA = this.composer.passes.some((pass) => {
      ...
      return (
        pass.enabled &&
        !pass.skipRendering &&
        ((_pass$effects = pass.effects) == null ? void 0 : _pass$effects.some((effect) => effect instanceof TRAAEffect))
      );
